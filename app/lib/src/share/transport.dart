...
/// called. Returns the running host (with the chosen WebSocket port).
Future<ShareHost> startHost({
  required String name,
  required SharedView initial,
}) =>
    impl.startHost(name: name, initial: initial);
...
abstract class ShareHost {
  ...
  /// LAN addresses others can use to join, as `ws://ip:port` URLs.
  List<String> get joinUrls;

  /// Live count of connected followers.
  Stream<int> get clientCount;

  /// Push a new shared view to all followers.
